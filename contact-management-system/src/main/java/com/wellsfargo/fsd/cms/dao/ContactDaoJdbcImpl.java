...
public class ContactDaoJdbcImpl implements ContactDao{
	...
	public static final String GET_CONT_QRY_BY_ID_QRY = "SELECT cid,cname,dob,mobile FROM Contacts WHERE cid=?";
	...
	public Contact save(Contact contact) throws ContactException {
		if(contact !=null){
			try(Connection con=ConnectionFactory.getConnection();
				...
				pst.setDate(1, Date.valueOf(contact.getDateOfBirth()));
				pst.setString(3, contact.getMobile());
				
				pst.executeUpdate();
				
			}catch(SQLException exp) {
				throw new ContactException("Updating Contact failed");
			}
		}
	...
	public Contact getById(int contactId) throws ContactException {
		...
		try(Connection con=ConnectionFactory.getConnection();
				PreparedStatement pst=con.prepareStatement(GET_CONT_QRY_BY_ID_QRY);
				){
